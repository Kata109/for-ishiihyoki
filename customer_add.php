<?php
	...
	if ($_POST["customer"]!="" && $_POST["nameA"]!="" && $_POST["dayNet"]!="" && $_POST["contacts"]!="" && $_POST["address"]!="" && $_POST["tel"]!="" && $_POST["consignee"]!="" && $_POST["consignee_address"]!="" && $_POST["consignee_tel"]!="" && $_POST["area"]!="") {
		//判断是否添加过
		$stmt = $conn->prepare("SELECT * FROM ihc_customer WHERE customer=?"); 
		$stmt->execute(array($_POST["customer"]));
		$rowC = $stmt->rowCount();
		if ($rowC==0) {
			$stmt = $conn->prepare("INSERT INTO ihc_customer(customer,nameA,dayNet,contacts,address,tel,consignee,consignee_address,consignee_tel,area) VALUES (?,?,?,?,?,?,?,?,?,?)");
			$stmt->execute(array($_POST["customer"],$_POST["nameA"],$_POST["dayNet"],$_POST["contacts"],$_POST["address"],$_POST["tel"],$_POST["consignee"],$_POST["consignee_address"],$_POST["consignee_tel"],$_POST["area"]));
			$rows = $stmt->rowCount();
			$conn = null;
			if ($rows) {
				$msg = "添加成功！";
