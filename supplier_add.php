<?php
	...
	if ($_POST["supplier"]!="" && $_POST["paymentDays"]!="" && $_POST["contacts"]!="" && $_POST["address"]!="" && $_POST["tel"]!="") {
		//判断是否添加过
		$stmt = $conn->prepare("SELECT * FROM ihc_supplier WHERE supplier=?"); 
		$stmt->execute(array($_POST["supplier"]));
		$rowC = $stmt->rowCount();
		if ($rowC==0) {
			$stmt = $conn->prepare("INSERT INTO ihc_supplier(supplier,paymentDays,contacts,address,tel) VALUES (?,?,?,?,?)");
			$stmt->execute(array($_POST["supplier"],$_POST["paymentDays"],$_POST["contacts"],$_POST["address"],$_POST["tel"]));
			$rows = $stmt->rowCount();
			$conn = null;
		if ($rows) {
			$msg = "添加成功！";
