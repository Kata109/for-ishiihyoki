<?php
	...
	//核对供应商是否在在库客户
	$stmt = $conn->prepare("SELECT * FROM ihc_supplier WHERE supplier='$supplier'"); 
	$stmt->execute();
	$row1 = $stmt->rowCount();

	if ($_POST["ma"]!="" && $_POST["nameC"]!="" && $_POST["nameJ"]!="" && $_POST["type"]!="" && $_POST["unit"]!="" && $row1 != 0 && is_numeric($_POST["price"]) && $_POST["byway"]!="" && $_POST["price"]>0) {
		//判断是否添加过
		$stmt = $conn->prepare("SELECT * FROM ihc_item WHERE ma=? AND supplier=?"); 
		$stmt->execute(array($_POST["ma"],$_POST["supplier"]));
		$rowC = $stmt->rowCount();
		if ($rowC==0) {
			$sql = "INSERT INTO ihc_item(ma,nameC,nameJ,type,unit,supplier,price,byway) VALUES (?,?,?,?,?,?,?,?)";
			$stmt = $conn->prepare($sql);
			$stmt->execute(array($_POST["ma"],$_POST["nameC"],$_POST["nameJ"],$_POST["type"],$_POST["unit"],$_POST["supplier"],$_POST["price"],$_POST["byway"]));
			$rows = $stmt->rowCount();
			$conn = null;
			if ($rows) {
				$msg = "添加成功！";
